...
def _parse_manual_from_env() -> List[str]:
    ...
    if not raw.strip():
        _warn_once(
            "manual_empty",
            "[watchlist] manual source enabled but WATCHLIST_TEXT is empty",
        )
        return []
    tokens: List[str] = []
...
def resolve_watchlist() -> Tuple[str, List[str]]:
    ...
    if requested not in _ALLOWED_SOURCES:
        _warn_once(
            ...
            "[watchlist] invalid WATCHLIST_SOURCE '%s'; using '%s' instead"
            % (requested, _DEFAULT_SOURCE),
        )
    ...
    try:
        ...
        try:
            ...
            if max_raw:
                _warn_once(
                    f"invalid_max:{max_raw}",
                    "[watchlist] MAX_WATCHLIST must be integer; ignoring value '%s'"
                    % max_raw,
                )
