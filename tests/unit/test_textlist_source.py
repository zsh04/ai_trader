...
def test_textlist_get_symbols_dedup(monkeypatch):
    fake_module = types.SimpleNamespace(
        get_symbols=lambda max_symbols=None: ["NVDA", "AMD", "NVDA"]
    )
...
def test_textlist_get_symbols_respects_max(monkeypatch):
    fake_module = types.SimpleNamespace(
        get_symbols=lambda max_symbols=None: ["NVDA", "AMD"]
    )
    monkeypatch.setenv("TEXTLIST_BACKENDS", "discord")
    monkeypatch.setattr(
        textlist_source, "_load_backend", lambda name: fake_module, raising=True
    )
