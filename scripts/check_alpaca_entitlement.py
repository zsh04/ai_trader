...
def check_entitlements():
    ...
    for feed in ("iex", "sip"):
        code, _, body = _req("/stocks/trades", {"symbol": "AAPL", "limit": 1, "feed": feed})
        if code == 200:
            ent[feed] = True
...
def main():
    ...
    if not API_KEY or not API_SECRET:
        print("ALERT: Alpaca API credentials not set (APCA_API_KEY_ID / APCA_API_SECRET_KEY).")
        sys.exit(2)
